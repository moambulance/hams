...
export class BookAnAmbulanceComponent implements OnInit {
  ...
  constructor(
    ...
    private comonService: CommonEventsService,
    ...
    // this.customerPatientAddForm.controls['hospital'].setValue(0);
  }

  ngOnInit(): void {
    // this.showSpinner();
    const admin_details: any = this.authService.getRole();
    this.hospital_id = 58;
    console.log('admin_details', admin_details);
    // this.hospital_id = admin_details.id;
  ...
  getHospitalById(event: any) {
    this.hospital_id = parseInt(event.target.value.split(': ')[1]);
  ...
  submitPayment() {
    ...
    // console.log('driverDetails details', this.driverDetails);
  ...
  mobileNumberPatch(event: any) {
    // console.log('okk', event.target.value);
  ...
  // }
  // get customer_name() {
  //   return this.bookAmbulanceForm.get('customer_name');
  // }
  // get lastName() {
